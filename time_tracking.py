...
def month_generator(callback):
    if callback.data == "last_month":
        ...
        if date_today.month - count_last_month < 1:
            ...
            count_last_month = 0
            count_month = 12
            first_day_of_month = calendar.monthrange(date_today.year - count_year, count_month - count_last_month)[0]
            last_day_of_month = calendar.monthrange(date_today.year - count_year, count_month - count_last_month)[-1]
        else:
            first_day_of_month = calendar.monthrange(date_today.year - count_year, count_month - count_last_month)[0]
            last_day_of_month = calendar.monthrange(date_today.year - count_year, count_month - count_last_month)[-1]
        ...
        bot.send_message(callback.message.chat.id,
                         f"Выбери дату в календаре:\n Текущая дата: {count_month - count_last_month}.{date_today.year - count_year}",
                         reply_markup=markup_days)
